 ...
 *
 * Arquitetura v3.4.2 — arquivo único:
...
const SYSTEM = Object.freeze({
  VERSION: '3.4.2', // exibida também no rodapé da interface
...
function catalogoCnds_() {
  return Object.freeze({
    ...
    'CND Municipal': {
      ...
      provider: 'manual',
      service: 'Sem API de Ipatinga no catálogo Infosimples',
...
function tentarCriarRascunhoCndsParaSolicitacaoNf_(candidate) {
  ...
  // A primeira leitura decide o que está efetivamente vencido. Ausentes não gastam API.
  const antes = obterSituacaoCndsParaCnpj_(cnpj);
  const renovacoes = renovarCndsVencidasAutomaticamente_(cnpj, antes.vencidas);

  // Releitura obrigatória após as tentativas: uma emissão bem-sucedida passa a ser anexo.
  ...
  const linhasCnds = validasComArquivo.map(item => {
    ...
    return '✓ ' + item.tipo + ' — válida até ' + formatarDataBr_(item.validade) + renovada;
  });
...
function formatarPendenciaCndParaRascunho_(item) {
  const lines = [
    ...
    '  Emitir: ' + (item.linkEmissao || 'link não configurado')
  ];
